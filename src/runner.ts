...
let _cachedPermissionMode: PermissionMode | null = null;

export function getPermissionMode(): PermissionMode {
  if (_cachedPermissionMode) return _cachedPermissionMode;
  try {
    const raw = JSON.parse(readFileSync(PERMISSION_MODE_FILE, "utf8")) as { mode?: string };
    if (raw.mode === "plan" || raw.mode === "acceptEdits" || raw.mode === "bypassPermissions") {
      _cachedPermissionMode = raw.mode;
      return raw.mode;
    }
  } catch {}
...
export function setPermissionMode(mode: PermissionMode): void {
  ...
  try {
    ...
    console.error("[runner] Failed to persist permission mode:", e);
  }
}

function buildSecurityArgs(security: SecurityConfig): string[] {
  const permMode = getPermissionMode();
  const args: string[] = permMode === "bypassPermissions"
