...
async function registerSlashCommands(token: string): Promise<void> {
  ...
  try {
    ...
    const builtinNames = new Set(["start", "reset", "compact", "status", "context"]);
    const commands = [
      ...
      { name: "compact", description: "Compact session to reduce context size", type: 1 },
      { name: "status", description: "Show current session status", type: 1 },
      { name: "context", description: "Show context window usage", type: 1 },
    ];
    for (const skill of skills) {
      ...
      const cmd = skill.name
        ...
        .replace(/[^a-z0-9\-_]/g, "")
        .slice(0, 32);
      if (!cmd || builtinNames.has(cmd)) continue;
